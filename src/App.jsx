...
const getThirdSaturday = (year, month) => {
  ...
  const firstSat = firstDayOfWeek === 6 ? 1 : 1 + ((6 - firstDayOfWeek + 7) % 7)
...
// ─── 이자 계산 엔진 ─────────────────────────────────────────
const calcInterest = (transactions, rate, allSD, lastSettlement, nextPaymentDate) => {
  const now = new Date()
  ...
  const sorted = [...transactions].sort((a, b) => parseLocal(a.date) - parseLocal(b.date))
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-blue-600 rounded-[2.5rem] p-8 shadow-2xl shadow-blue-200 mb-4 mt-4 relative overflow-hidden text-white">
          <div className="relative z-10">
            ...
            <div className="text-5xl font-black font-mono tracking-tighter mb-8">
              {liveAccrued.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              <span className="text-xl ml-1 font-sans font-normal opacity-80">원</span>
            </div>
            <div className="flex justify-between items-end border-t border-white/20 pt-6">
              <div>
                <p className="text-blue-200 text-[10px] font-bold uppercase">이번 분기 예상 총 이자</p>
                <p className="text-xl font-bold font-mono">{Math.floor(expectedTotal).toLocaleString()}원</p>
              </div>
              <div className="flex flex-col items-end gap-1">
                <span className="bg-white/20 text-[10px] font-bold px-2 py-0.5 rounded-md">D-{dDay}</span>
                <span className="text-[10px] font-medium text-blue-100 opacity-80">
                  {settlementInfo.nextPaymentDate?.toLocaleDateString('ko-KR', { month: 'long', day: 'numeric' })} 지급
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
            <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">현재 잔액</p>
            <p className="text-lg font-black text-slate-700">{currentBalance.toLocaleString()}원</p>
          </div>
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
            <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">적용 금리 설정</p>
            <div className="flex items-center gap-1">
              <input type="number" step="0.1" value={rate} onChange={(e) => setRate(Number(e.target.value))} className="text-lg font-black text-blue-600 bg-transparent outline-none w-16" />
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm mb-6 border border-slate-100">
          <div className="space-y-4">
            <div className="flex gap-2">
              <input type="date" value={inputDate} onChange={(e) => setInputDate(e.target.value)} className="flex-1 bg-slate-50 p-3 rounded-xl text-sm font-bold outline-none" />
              <input type="number" placeholder="금액 입력" value={inputAmount} onChange={(e) => setInputAmount(e.target.value)} className="flex-[1.5] bg-slate-50 p-3 rounded-xl text-sm font-bold outline-none" />
            </div>
            <div className="flex gap-2">
              <button onClick={() => addTransaction('deposit')} className="flex-1 bg-slate-800 text-white py-3.5 rounded-xl font-bold active:scale-95 transition-all text-sm">입금 (+)</button>
              <button onClick={() => addTransaction('withdraw')} className="flex-1 bg-slate-100 text-slate-600 py-3.5 rounded-xl font-bold active:scale-95 transition-all text-sm">출금 (-)</button>
            </div>
          </div>
        </div>
          ...
          {[...transactions].reverse().map((t) => (
            <div key={t.id} className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border border-slate-50">
              ...
              <div className="flex items-center gap-3">
                <p className={`font-bold ${t.amount > 0 ? 'text-blue-600' : 'text-rose-500'}`}>
                  {t.amount > 0 ? '+' : ''}{t.amount.toLocaleString()}원
                </p>
                <button onClick={() => setTransactions(transactions.filter(item => item.id !== t.id))} className="text-slate-300 hover:text-rose-400">×</button>
              </div>
            </div>
          ))}
        </div>
